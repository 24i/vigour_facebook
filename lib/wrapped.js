  ...
  define: {
    init (appId) {
      ...
      fb.send('init', appId, (err, response) => {
        console.log('init response!', err, response)
        console.log('I now have conenctionStatus', fb.connectionStatus.val)
        if (!err) {
          fb.set(response)
        }
        console.log('it should have set connectionStatus', fb.connectionStatus.val)
        fb.ready.val = true
      })
    },
    _login (callback) {
