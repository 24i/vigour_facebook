'use strict'

var Observable = require('vigour-js/lib/observable')

// TODO: get this stuff from config:
var SDK_SRC = '//connect.facebook.net/en_NL/sdk.js'
var SDK_ID = 'facebook-jssdk'
var SCOPE = 'public_profile,email'

module.exports = new Observable({
  key: 'Facebook',
  token: false,
  ready: false,
  connectionStatus: false,
  queue: {
    useVal: {}
  },
  on: {
    new () {
      var facebook = this
      if (!facebook.appId) {
        throw new Error('Facebook: needs an appId')
        // implement ability to set it later?
      }
      facebook.init()
    }
  },
  define: {
    init () {
      var fb = this
      // set up window callback for SDK script loaded
      window.fbAsyncInit = function fbAsyncInit () {
        console.log('window.fbAsyncInit()!')
        fb.ready.val = true
        FB.init({
          appId: fb.appId.val,
          xfbml: fb.xfbml ? fb.xfbml.val : true,
          version: fb.version ? fb.version.val : 'v2.5'
        })
        FB.getLoginStatus(function (response) {
          fb.loginResponse(response)
        })
        // do queued stuff
        for (let q in fb.queue) {
          let method = fb[q]
          method.apply(fb, fb.queue[q])
        }
      }
      // insert script
      var scriptElement = makeScriptElement(SDK_SRC, SDK_ID)
      document.getElementsByTagName('head')[0].appendChild(scriptElement)
    },
    login (callback) {
      console.log('facebook.login()!')
      var fb = this
      if (fb.ready.val) {
        FB.login((response) => {
          fb.loginResponse(response)
          if (response.status !== 'connected') {
            fb.emit('error', 'login failed')
          }
          // if callback do callback
          if (callback) {
            callback(null, response)
          }
        }, {scope: SCOPE})
      } else {
        if (fb.queue.logout) {
          delete fb.queue.logout
        }
        fb.queue.login = [callback]
      }
    },
    logout (callback) {
      console.log('facebook.logout()')
      var fb = this
      fb.set({
        token: false,
        userID: false,
        connectionStatus: 'unknown'
      })
      if (fb.ready.val) {
        FB.logout((response) => {
          console.log('logout response', response)
        })
      } else {
        console.warn('FB not yet loaded, queue that login!')
        // TODO: queue that login
        if (fb.queue.login) {
          delete fb.queue.login
        } else {
          fb.queue.logout = [callback]
        }
      }
    },
    loginResponse (response) {
      var fb = this
      var setobj = {
        connectionStatus: response.status
      }
      if (response.status === 'connected') {
        console.log('yes connected!')
        var auth = response.authResponse
        setobj.token = auth.accessToken
        setobj.userID = auth.userID
        // setobj.signedRequest: auth.signedRequest
      }
      fb.set(setobj)
    },
    share (shared, callback) {
      FB.ui({
        method: 'share',
        href: shared
      }, function (response) {
        console.log('FB SHARE callback', response)
        if (callback) {
          callback(null, response)
        }
      })
    }
  }
}).Constructor

function makeScriptElement (src, id) {
  var script = document.createElement('script')
  script.onerror = () => {
    this.emit('error', 'script load error! src:' + src + ' id:' + id)
  }
  script.src = src
  script.id = id
  return script
}
